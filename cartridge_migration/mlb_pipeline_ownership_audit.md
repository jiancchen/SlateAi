...
- `models/mlb/cartridges/MLB-M0/` owns daily model behavior: prediction lanes, workflow orchestration, model scoring code, model notes, run locks, and golden checks.
...
| `pipeline/mlb/publish/` | Compatibility launchers | Temporarily | Canonical lane implementations now live in `models/mlb/cartridges/MLB-M0/lanes/`. | Remove after direct callers are audited or formally keep as stable CLI shims. |
...
| `pipeline/mlb/workflows/` | Compatibility launchers | Temporarily | Canonical workflows now live in `models/mlb/cartridges/MLB-M0/workflows/`. | Remove after direct callers are audited or formally keep as stable CLI shims. |
