...
| MLB-M0 model lib | `models/mlb/cartridges/MLB-M0/lib/sports-model.js` still contains the public factory, generic/UFC context, fallback analysis, and final structured-analysis orchestration. | It remains the stable public API while model-owned internals are split behind it. | Future improvements may still patch the orchestrator when they belong in a lane module. | Split remaining generic context/orchestration only after MLB behavior stays golden and shared-vs-MLB ownership is clear. | MLB context, decision indicators, props, starter helpers, market helpers, signal helpers, side controls, simulation, pick ranking, and parlay helpers are split; next candidate is structured-analysis orchestration. | Open |
