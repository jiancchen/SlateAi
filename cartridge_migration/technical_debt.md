...
| MLB-M0 run locks | File-based locks live under `data-private/model-runs/mlb/MLB-M0/` and are indexed into shared `model_runs` tables. | The JSON lock remains the reproducibility artifact, while the DB is the query layer. | Two sources can drift if a future lock path forgets to re-index. | Every lock/update path runs `models/shared/model-runs/index_runs.py`; tests assert DB rows exist for locked runs. | Keep run-lock indexing mandatory as new models are added. | Open |
| MLB-RP36 addendum | `models/mlb/cartridges/MLB-RP36/` has file locks plus RP36 settlement rows in the shared warehouse. | RP36 is an input component, not the parent daily slate runner. | Exact first-up identity can be overinterpreted as a betting model. | RP36 remains component-scored with exact/top-2/top-3 lanes, consumed by MLB-M0 instead of promoted as a parent. | Backtest RP36 by component lane, not as a standalone pick engine. | Open |
