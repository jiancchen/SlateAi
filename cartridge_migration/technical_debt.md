...
| MLB-M0 model lib | `models/mlb/cartridges/MLB-M0/lib/sports-model.js` is still a large mixed module. | It was moved behavior-preserving first to keep golden snapshots stable. | MLB analysis context, props, and generic match helpers remain hard to reason about. | Split into smaller lane modules while preserving the exported API and passing May 30/May 31 golden checks. | Core utility, team-name, simulation, pick-ranking, side-control, and parlay helpers are split; continue with MLB analysis context, then props. | Open |
