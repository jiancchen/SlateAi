...
const gitInfo = async () => {
  ...
  return {
    ...
    changedFiles: dirty.ok ? dirty.stdout.split('\n').filter(Boolean) : []
  }
}
...
const sourceInventory = async () => {
  ...
  const manifest = await readJson('models/mlb/cartridges/M0/manifest.json', {})
  const rp36 = await readJson('models/mlb/cartridges/RP36/manifest.json', {})
  const e0 = await readJson('models/mlb/cartridges/E0/manifest.json', {})
  return uniqueEntries([
    { path: 'models/mlb/registry.json', role: 'model-registry' },
    { path: 'models/registry.json', role: 'top-level-model-registry' },
    { path: 'models/mlb/cartridges/M0/manifest.json', role: 'model-manifest' },
    ...
    { path: 'models/mlb/cartridges/M0/verify_run.mjs', role: 'run-verifier' },
    { path: 'models/mlb/cartridges/RP36/manifest.json', role: 'relief-addendum-manifest' },
    ...
    { path: 'models/mlb/cartridges/E0/manifest.json', role: 'evaluator-manifest' },
...
export const lockM0Run = async ({ date }) => {
  ...
  const runId = `mlb-${date}-${active.warehouse || 'W1'}-${active.features || 'F0'}-${active.model || 'M0'}-${active.reliefAddendum || 'RP36'}-${active.evaluator || 'E0'}`
  const runDir = `data-private/model-runs/mlb/M0/${date}`
  ...
  const run = {
    ...
    sport: 'mlb',
    slateDate: date,
    modelId: active.model || 'M0',
    reliefAddendum: active.reliefAddendum || 'RP36',
