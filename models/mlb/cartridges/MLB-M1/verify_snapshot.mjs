...
import { buildM1Snapshot, snapshotPath } from './snapshot.mjs'
...
const actual = await buildM1Snapshot(options)
const expectedPath = snapshotPath(options)
const expected = await readJson(expectedPath)
const diff = firstDiff(actual, expected)

if (diff) {
  const actualPath = `data-private/model-cartridges/mlb/MLB-M1/golden/${options.date}.actual.json`
  ...
  throw new Error(`MLB-M1 snapshot mismatch for ${options.date}: ${diff}. Actual written to ${actualPath}`)
}
