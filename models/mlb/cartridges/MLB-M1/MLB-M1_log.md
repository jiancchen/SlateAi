...
- MLB-M1 source locks no longer include workflow/publish compatibility launchers or the frontend compatibility shim.
- `models/shared/model-runs/index_runs.py` writes locked MLB-M1/MLB-RP36 runs into shared warehouse model-run tables.
...
Every new MLB-M1/MLB-RP36 lock should re-index the warehouse rows, and tests should fail if locked runs stop producing DB model-run/lane rows.
...
- Good enough to run, lock, verify, and compare the active `MLB-M1` through the registry.
...
- MLB-M1 runner and run-lock self-inventory now use local cartridge paths where possible.

Remaining boundary:

Direct MLB-M1 and MLB-RP36 file paths are still expected inside manifests, component inventory, and run locks. Those paths describe the cartridge contents; they are not operator entrypoints.
