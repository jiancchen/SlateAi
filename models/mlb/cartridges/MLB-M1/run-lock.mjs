...
export const lockM1Run = async ({ date, model = process.env.MLB_MODEL_ID || localModelId }) => {
  const registry = await readJson('models/mlb/registry.json', {})
  ...
  if (!indexResult.ok) {
    throw new Error(`Failed to index MLB-M1 run in warehouse: ${indexResult.stderr || indexResult.stdout}`)
  }
...
if (import.meta.url === executedUrl) {
  ...
  const { run, runDir } = await lockM1Run({ date, model })
