...
const compactGame = (game = {}) => ({
  ...
  gameShape: game.analysis?.gameShape
    ? {
        ...
        rfLens: {
          mode: game.analysis.gameShape.rfLens?.mode ?? null,
          impactScore: round(game.analysis.gameShape.rfLens?.impactScore),
          trustedUse: game.analysis.gameShape.rfLens?.trustedUse ?? null,
          totalsRead: game.analysis.gameShape.rfLens?.totalsRead ?? null
        }
      }
    : null,
...
export const buildM2Snapshot = async ({ date }) => {
  ...
  const gameFiles = (await fs.readdir(gamesDir))
    ...
    .sort()
  const games = await Promise.all(gameFiles.map(async (fileName) => {
    const rawGame = await readJson(`published-data/slates/${date}/games/${fileName}`, {})
    return compactGame(createSportsMatchModel(rawGame))
  }))
...
export const snapshotPath = ({ date }) => `data-private/model-cartridges/mlb/MLB-M2/golden/${date}.snapshot.json`
...
if (import.meta.url === executedUrl) {
  ...
  const snapshot = await buildM2Snapshot(options)
  if (options.update) {
