...
import { buildM2Snapshot, snapshotPath } from './snapshot.mjs'
...
const actual = await buildM2Snapshot(options)
const expectedPath = snapshotPath(options)
const expected = await readJson(expectedPath)
const diff = firstDiff(actual, expected)

if (diff) {
  const actualPath = `data-private/model-cartridges/mlb/MLB-M2/golden/${options.date}.actual.json`
  ...
  throw new Error(`MLB-M2 snapshot mismatch for ${options.date}: ${diff}. Actual written to ${actualPath}`)
}
