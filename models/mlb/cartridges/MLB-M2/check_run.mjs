...
const main = async () => {
  ...
  const runDir = `data-private/model-runs/mlb/MLB-M2/${date}`
  const run = await readJson(`${runDir}/run.json`)
  const snapshot = await readJson(`${runDir}/snapshot.json`)
  const actualSnapshot = await buildM2Snapshot({ date })
  ...
  if (diff) {
    ...
    throw new Error(`MLB-M2 run snapshot mismatch: ${diff}. Actual written to ${actualPath}`)
  }
