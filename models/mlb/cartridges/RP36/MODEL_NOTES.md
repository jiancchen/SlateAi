# RP36 Reliever Shadow Addendum

RP36 is the cartridge name for the current MLB relief stack, legacy alias `E36 shadow`.

It should stay as an addendum consumed by M0. Its job is to explain bullpen path, first-up reliever likelihood, availability, and bridge risk.

## Scope

- First-up reliever shortlist
- Top-2 and top-3 reliever cluster confidence
- Heavy-use and quick-reuse reset
- Bridge risk and bullpen shape context
- Late-game warnings for M0 side, total, and prop decisions

## Migration Rule

The first real RP36 migration must reproduce the existing 2026-05-30 reliever-shadow artifact before any scoring changes are accepted.

## Known Gaps

- No RP36 settlement table yet.
- Current exact first-up hit rate is not strong enough for standalone bets.
- The payload still uses `E36 shadow` as a display/model tag.
