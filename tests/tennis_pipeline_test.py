...
class TennisWarehouseImportTest(unittest.TestCase):
    ...
    def test_ten_t0_postmatch_settlement_artifact_has_lane_rows(self) -> None:
        ...
        self.assertEqual(artifact.get("status"), "settled")
        self.assertEqual(artifact.get("completeMatches"), 8)
        self.assertEqual(artifact.get("pendingMatches"), 0)
        self.assertEqual(artifact.get("rowCount"), 48)
        self.assertEqual(artifact.get("gradedCount"), 27)
        self.assertEqual(artifact.get("hitCount"), 15)
        self.assertEqual(lanes.get("ML", {}).get("rows"), 8)
        self.assertEqual(lanes.get("ML", {}).get("graded"), 8)
        self.assertEqual(lanes.get("ML", {}).get("hits"), 6)
        self.assertEqual(lanes.get("Spread", {}).get("rows"), 8)
        self.assertEqual(lanes.get("Spread", {}).get("graded"), 8)
        self.assertEqual(lanes.get("Match O/U", {}).get("rows"), 8)
        self.assertEqual(lanes.get("Match O/U", {}).get("graded"), 4)
        self.assertEqual(lanes.get("First-set O/U", {}).get("rows"), 8)
        self.assertEqual(lanes.get("First-set O/U", {}).get("graded"), 7)
