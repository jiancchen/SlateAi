...
TARGET = RESEARCH_DIR / "research_mlb_first5_state_model.py"


def _load_target() -> None:
    sys.path.insert(0, str(RESEARCH_DIR))
    spec = importlib.util.spec_from_file_location("_research_mlb_first5_state_model_py_impl", TARGET)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load {TARGET}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["_research_mlb_first5_state_model_py_impl"] = module
