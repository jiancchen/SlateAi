...
const BLOCKED_TENNIS_SOURCE_PATTERN = /flashscore|sofascore|tennistonic|tennis[_\s-]*tonic/i
...
const parseArgs = () => {
  ...
  const options = {
    ...
    predictionsOutput: '',
    model: 'TEN-T0',
    inputSource: process.env.TENNIS_T0_USE_DB === '1' ? 'db' : 'files',
    dbPath: process.env.SLATE_TENNIS_WAREHOUSE_DB || DEFAULT_TENNIS_DB,
    warehouseContext: ''
  }
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === '--date') {
      ...
      index += 1
    } else if (arg === '--warehouse-context') {
      options.warehouseContext = args[index + 1] || ''
      index += 1
    }
  }
  ...
  options.warehouseContext ||= `web/src/lib/day-${options.date}-tennis-warehouse-context.generated.json`
  return options
}
...
const playerIdentityKeys = (name) => {
  ...
  const aliasMap = {
    ...
    'noma akugue noha': ['noma noha akugue'],
    'noma noha akugue': ['noma akugue noha'],
    'diego dedura': ['diego dedura palomero'],
    'diego dedura palomero': ['diego dedura']
  }
  return Array.from(new Set([key, ...(aliasMap[key] || [])]))
}
...
const findWarehousePlayer = (warehouse, matchId, name) => {
  ...
  const keys = playerIdentityKeys(name)
  const matchPlayer = players.find((player) => keys.includes(normalizeName(player.name)))
  if (matchPlayer) return matchPlayer
  const playerByName = keys.map((key) => warehouse?.playersByName?.[key]).find(Boolean)
  if (
    playerByName?.profile ||
    playerByName?.ranking ||
    playerByName?.expectedStats ||
...
const clayRecordScore = (qualityPlayer, surface = 'Unknown') => {
...
const playerScore = (ranking, qualityPlayer, isAtp, surface = 'Unknown') =>
  rankScore(ranking) +
...
const buildRobinhoodMarketData = ({ robinhood, players, pickName }) => {
  const sourceText = `${robinhood?.source || ''} ${robinhood?.sourceUrl || ''}`
  if (!/robinhood/i.test(sourceText)) return null
...
const formatProfile = (ranking, qualityPlayer, surface = 'Unknown') => {
  ...
  const parts = [
    ...
    clayRecord && !isClaySurface(surface) ? `${surface || 'Surface'} match; surface-specific record unavailable` : null,
...
const buildRead = ({ pick, opponent, confidence, volatility, pickQuality, oppQuality, isAtp, surface = 'Unknown' }) => {
  ...
  if (!notes.length) {
    notes.push(
      isClaySurface(surface)
        ...
        : `${pick} has the cleaner composite of rank, recent opponent quality, and joined service data on ${surface || 'this surface'}.`
    )
  }
  const discipline =
...
const buildValueBoard = ({ marketData, pickName, confidence, volatility, weaknessEdge, setWinProjections, derivativeCase, totalsProfile }) => {
  ...
  const mlCandidateFor = (player) => {
    ...
    const issueText = valueIssue({ marketType: 'ML', edgePct, ev: candidateEv, netEv: candidateNetEv, odds, modelPct })
    const gradeText = valueGrade({ marketType: 'ML', edgePct, ev: candidateEv, odds, modelPct })
    const suppressRawEv = /outlier|manual|model-market/i.test(`${issueText} ${gradeText}`)
    return {
      marketType: 'ML',
      ...
      impliedPct: Number.isFinite(impliedPct) ? Number(impliedPct.toFixed(1)) : null,
      edgePct: suppressRawEv ? null : Number.isFinite(edgePct) ? Number(edgePct.toFixed(1)) : null,
      evPer100: suppressRawEv ? null : candidateEv,
      netEvPer100: suppressRawEv ? null : candidateNetEv,
      feePer100: FEE_PER_100_RISKED,
      rawEdgePct: Number.isFinite(edgePct) ? Number(edgePct.toFixed(1)) : null,
      rawEvPer100: candidateEv,
      rawNetEvPer100: candidateNetEv,
      valueIssue: issueText,
      valueGrade: gradeText,
  ...
  const bestOpponentMl = (marketData.players || [])
    ...
    .map(mlCandidateFor)
    .filter((row) => Number.isFinite(Number(row.rawEdgePct ?? row.edgePct)) || Number.isFinite(Number(row.rawEvPer100 ?? row.evPer100)))
    .sort((left, right) => Math.max(Number(right.rawEdgePct ?? right.edgePct) || -99, Number(right.rawEvPer100 ?? right.evPer100) || -99) - Math.max(Number(left.rawEdgePct ?? left.edgePct) || -99, Number(left.rawEvPer100 ?? left.evPer100) || -99))[0]
  ...
  const mlValueRow =
    pickIsTaxed && bestOpponentMl && (Number(bestOpponentMl.rawEdgePct ?? bestOpponentMl.edgePct) > 0 || Number(bestOpponentMl.rawEvPer100 ?? bestOpponentMl.evPer100) > 0)
      ? {
          ...bestOpponentMl,
          valueIssue: /outlier|manual|model-market/i.test(String(bestOpponentMl.valueGrade || bestOpponentMl.valueIssue || ''))
            ? `Outlier dog price on ${bestOpponentMl.selection}; raw model-vs-price EV is hidden until matchup, line source, and probability are manually validated.`
            : `Picked side is overpriced; value board flips to ${bestOpponentMl.selection} because that side has the better posted-price edge.`,
          betGrade: false
        }
      : pickedMl
...
const buildBettingMatrix = ({ marketData, valueBoard, setWinProjections, derivativeCase, pickName, confidence }) => {
  ...
  if (ml) {
    matrix.push({
      ...
      edgePct: ml.edgePct ?? (ml.selection === pickName ? marketData?.desk?.edgePct ?? null : null),
      evPer100: ml.evPer100 ?? null,
      netEvPer100: ml.netEvPer100 ?? null,
      grade: ml.valueGrade || 'No price',
      issue: ml.valueIssue || null,
      reason:
        /outlier|manual|model-market/i.test(String(ml.valueGrade || ml.valueIssue || ''))
          ? 'Outlier ML price: do not treat the raw model gap as actionable EV without manual matchup, line-source, and probability validation.'
...
const buildGame = (match, rankings, quality, warehouse, date, sportsbookIndex, robinhoodIndex, ensembleValueIndex, ensembleRowsByMatch, derivativeIndex) => {
  ...
  const rankA = warehouseRanking(warehouseA) ?? getRanking(rankings, a.name)
  const rankB = warehouseRanking(warehouseB) ?? getRanking(rankings, b.name)
  ...
  const robinhoodMarketData = buildRobinhoodMarketData({ robinhood, players, pickName })
  const marketData = sportsbookMarketData ?? robinhoodMarketData
  ...
  return {
    ...
    ensembleValueCase,
    marketData,
    predictionMarketData: robinhoodMarketData,
    h2hUrl: null,
    researchLinks: [
      ...
      robinhoodMarketData?.eventUrl ? { label: 'Robinhood market', url: robinhoodMarketData.eventUrl } : null
    ].filter(Boolean),
    players
  }
}
...
const buildSupplementGame = (match, rankings, warehouse, sportsbookIndex) => {
  ...
  const categoryText = String(match.category || '')
  const tour = /wta|women/i.test(categoryText) ? 'WTA' : 'ATP'
  const isAtp = tour === 'ATP'
  const eventTag = /challenger/i.test(categoryText)
    ? `${tour} Challenger`
    : /french_open/i.test(categoryText)
      ? `French Open ${tour === 'WTA' ? 'Women' : 'Men'}`
      : tour
  const tournamentLabel = match.tournament || eventTag
  ...
  const sportsbook = findSportsbookLine(sportsbookIndex, a.name, b.name)
  const gameId = sportsbook?.syntheticSlateId || match.id
  ...
  const warehouseA = findWarehousePlayer(warehouse, gameId, a.name)
  const warehouseB = findWarehousePlayer(warehouse, gameId, b.name)
  const rankA = warehouseRanking(warehouseA) ?? getRanking(rankings, a.name)
  const rankB = warehouseRanking(warehouseB) ?? getRanking(rankings, b.name)
  ...
  const weaknessEdge = hasWarehouseDepth
    ...
    : {
        ...
        attackingSide: null,
        vulnerableSide: null,
        gameFlow: `Market is captured, but rank/form/service data has not been joined for this ${eventTag} row yet.`,
  ...
  const read = hasWarehouseDepth
    ? buildRead({ pick: pickName, opponent: opponentName, confidence, volatility, pickQuality, oppQuality, isAtp, surface })
    : {
        reason: `${pickName} is only the current market favorite over ${opponentName}; ${surface} surface is tagged, but no surface-specific warehouse service, break-point, or opponent-quality edge is joined yet.`,
        totals: `No posted sportsbook total captured for this ${eventTag} market.`
      }
  const players = [
    {
      name: a.name,
  ...
  const sportsbookMarketData = buildMarketData({ sportsbook, players, pickName, weaknessEdge, confidence, totals: read.totals })
  const robinhoodMarketData = buildRobinhoodMarketData({ robinhood: match, players, pickName })
  const marketData = sportsbookMarketData ?? robinhoodMarketData
  ...
  const totalsProfile = hasWarehouseDepth
    ? buildTotalsProfile({ tour, confidence, volatility, qualityA, qualityB, weaknessA, weaknessB })
    : null
  ...
  return {
    ...
    eventId: match.eventId,
    tour,
    bestOf: categoryText === 'french_open_men_singles' ? 5 : 3,
    surface,
    ...
    court: tournamentLabel,
    round: match.round || 'Round',
    stage: `${tournamentLabel} | ${match.round || 'Round'}`,
    pickName,
    basePickName: pickName,
    modelSource: hasWarehouseDepth ? `TennisLive recent-five ${eventTag} model` : 'Sportsbook market watch only',
    modelSplit: false,
    ...
    tags: [
      eventTag,
      surface,
      match.predictionMarket?.source || 'Sportsbook board',
...
const sportsbookSupplementShape = (row, date) => {
  ...
  const leagueText = `${leagueName} ${row.href || ''}`.toLowerCase()
  const tour = leagueText.includes('wta') || leagueText.includes('women') ? 'WTA' : 'ATP'
  const category = leagueText.includes('french open') || leagueText.includes('roland garros')
    ? tour === 'WTA'
      ? 'french_open_women_singles'
      : 'french_open_men_singles'
    : leagueText.includes('challenger')
      ? `${tour.toLowerCase()}_challenger_singles`
      : tour === 'WTA'
        ? 'wta_singles'
        : 'atp_singles'
...
const isPrioritySportsbookRow = (row) => {
...
const moduleImportPath = (fromFilePath, toFilePath) => {
  const fromDir = path.dirname(path.resolve(ROOT, fromFilePath))
  const toResolved = path.resolve(ROOT, toFilePath)
  const relativePath = path.relative(fromDir, toResolved).replace(/\\/g, '/')
  return relativePath.startsWith('.') ? relativePath : `./${relativePath}`
}

const assertNoBlockedTennisSources = (label, payload) => {
  const text = typeof payload === 'string' ? payload : JSON.stringify(payload)
  const match = BLOCKED_TENNIS_SOURCE_PATTERN.exec(text || '')
  if (!match) return
  const start = Math.max(0, match.index - 90)
  const end = Math.min(text.length, match.index + match[0].length + 90)
  throw new Error(`Blocked legacy tennis source leaked into ${label}: ...${text.slice(start, end).replace(/\s+/g, ' ')}...`)
}
...
const main = async () => {
  ...
  const scoreboard = useDbInputs
    ...
    : await readJson(`data-private/reference/tennis/espn-scoreboard-${options.date}.json`, { singles: [] })
  ...
  const warehouseContext = await readJson(options.warehouseContext, { matches: {} })
  ...
  const draftKingsSupplementGames = (draftkingsLines.matches || [])
    .filter(isPrioritySportsbookRow)
  ...
  const sportsModelImportPath = moduleImportPath(options.output, 'web/src/lib/sports-model.js')
  const warehouseContextImportPath = moduleImportPath(options.output, options.warehouseContext)
  ...
  moduleText = moduleText.replace(
    '  const market = raw.marketData ?? null\n',
    "  const market = raw.marketData ?? null\n  const predictionMarketLine = raw.predictionMarketData ?? (market?.source === 'Robinhood prediction market' ? market : null)\n"
  )
  moduleText = moduleText.replace(
    '  const marketPlayers = market?.players ?? []\n  const deskMarket = market?.desk ?? null\n',
    '  const marketPlayers = market?.players ?? []\n  const deskMarket = market?.desk ?? null\n  const predictionMarketPlayers = predictionMarketLine?.players ?? []\n'
  )
  moduleText = moduleText.replace(
    "  const predictionMarket = market ? {\n    source: market.source,\n    capturedAt: market.capturedAt,\n    totalVolume: null,\n    players: marketPlayers.map((player) => ({ name: player.name, probabilityPct: player.impliedPct, amount: null, americanOdds: player.odds, edgePct: player.edgePct, priceBand: player.priceBand }))\n  } : null\n",
    "  const predictionMarket = predictionMarketLine ? {\n    source: predictionMarketLine.source,\n    capturedAt: predictionMarketLine.capturedAt,\n    eventUrl: predictionMarketLine.eventUrl,\n    eventId: predictionMarketLine.eventId,\n    totalVolume: predictionMarketLine.totalVolume ?? null,\n    totalOpenInterest: predictionMarketLine.totalOpenInterest ?? null,\n    players: predictionMarketPlayers.map((player) => ({\n      name: player.name,\n      probabilityPct: player.impliedPct,\n      bidPct: player.bidPct ?? null,\n      lastTradePct: player.lastTradePct ?? null,\n      amount: player.openInterest ?? null,\n      americanOdds: player.odds,\n      edgePct: player.edgePct,\n      priceBand: player.priceBand,\n      symbol: player.symbol ?? null\n    }))\n  } : null\n"
  )
  moduleText = moduleText.replace(
    "    market ? { label: 'FanDuel moneyline', book: market.source, value: market.mlValue } : null,\n",
    "    market ? { label: 'FanDuel moneyline', book: market.source, value: market.mlValue } : null,\n    predictionMarketLine && predictionMarketLine.source !== market?.source ? { label: 'Prediction market prices', book: predictionMarketLine.source, value: predictionMarketLine.mlValue } : null,\n"
  )

  moduleText = moduleText.replace(
    `import { createSportsMatchModel } from './sports-model.js'\n`,
    `import { createSportsMatchModel } from '${sportsModelImportPath}'\n`
  )
  moduleText = moduleText.replace(
    `import tennisWarehouseContext from './day-${options.date}-tennis-warehouse-context.generated.json' with { type: 'json' }\n`,
    `import tennisWarehouseContext from '${warehouseContextImportPath}' with { type: 'json' }\n`
  )
  moduleText = moduleText.replace(
    `import tennisClayContext from './day-${options.date}-tennis-clay-context.generated.json' with { type: 'json' }\n`,
    ''
  ...
  assertNoBlockedTennisSources(`raw tennis games ${options.date}`, games)
  assertNoBlockedTennisSources(`generated day module ${options.date}`, moduleText)
    ...
    `${JSON.stringify(
      {
        ...
        picks: games.map((game, index) => ({
          ...
          predictionMarket: game.predictionMarketData
            ? {
                source: game.predictionMarketData.source,
                capturedAt: game.predictionMarketData.capturedAt,
                eventUrl: game.predictionMarketData.eventUrl,
                totalOpenInterest: game.predictionMarketData.totalOpenInterest,
                totalVolume: game.predictionMarketData.totalVolume,
                moneyline: game.predictionMarketData.players.map((player) => ({
                  name: player.name,
                  probabilityPct: player.impliedPct,
                  bidPct: player.bidPct,
                  lastTradePct: player.lastTradePct,
                  openInterest: player.openInterest,
                  symbol: player.symbol,
                  modelPct: player.modelPct,
                  edgePct: player.edgePct
                })),
                priceAction: game.predictionMarketData.priceAction
              }
            : null,
