...
TARGET = ROOT / "pipeline" / "mlb" / "warehouse" / "mlb_warehouse.py"


def _load_target() -> None:
    spec = importlib.util.spec_from_file_location("_mlb_warehouse_impl", TARGET)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load {TARGET}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["_mlb_warehouse_impl"] = module
