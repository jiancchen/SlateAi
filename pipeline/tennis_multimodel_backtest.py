...
TARGET = ROOT / "pipeline" / "tennis" / "research" / "multimodel_backtest.py"


def _load_target() -> None:
    spec = importlib.util.spec_from_file_location("_tennis_multimodel_backtest_impl", TARGET)
    if spec is None or spec.loader is None:
        raise ImportError(f"Unable to load {TARGET}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["_tennis_multimodel_backtest_impl"] = module
