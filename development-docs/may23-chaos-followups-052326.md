...
- Make sure closeout writes `data-private/history/mlb-results-2026-05-23.jsonl`.
- Make sure imported side boards actually flow into `mlb_side_backtests`.

Reason:
- today had `14` board picks
- only `13` side-prediction rows imported
- `mlb_side_backtests` still shows `0` rows for the May 23 v1.1 board
...
This slate produced `5` side misses where the predicted team simply never got going:
- Pirates @ Blue Jays, White Sox @ Giants, Mariners @ Royals, Nationals @ Braves, Athletics @ Padres
