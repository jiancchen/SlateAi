...
6. Warehouse supplemental model context:
   ...
   - MLB-SP1 starter profile addendum with `npm run data:build:mlb-sp1 -- --date YYYY-MM-DD`, then `npm run data:audit:mlb-sp1 -- --date YYYY-MM-DD`. Run it after canonical hitter/pitcher split-family warehousing and before day-file generation. The morning contract and causal-ledger audits must fail if SP1 is missing or not attached. See [mlb-starter-profile-addendum-runbook.md](/Users/jcchen/Documents/New%20project/development-docs/mlb/runbooks/mlb-starter-profile-addendum-runbook.md:1).
