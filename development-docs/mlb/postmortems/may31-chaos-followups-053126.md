...
- Make sure closeout writes `data-private/history/mlb-results-2026-05-31.jsonl`.
...
This slate produced `2` side misses where the predicted team simply never got going:
- Marlins @ Mets, Royals @ Rangers
...
### P1: Fix RP36 post-closeout reproducibility
The MLB-M0 run snapshot/check passed after closeout, but MLB-RP36 did not reproduce exactly after the warehouse was refreshed with final game data:

- stored RP36 snapshot candidates: `276`
- regenerated check candidates: `271`

That does not mean the pregame RP36 read was wrong, but it means the check is currently using mutable postgame warehouse state. RP36 needs either:

- a pregame input snapshot that check can replay exactly, or
- a postgame check mode that validates stored artifact shape without regenerating from mutated inputs.

Do not treat postgame RP36 regeneration as a stable benchmark until this is fixed.
