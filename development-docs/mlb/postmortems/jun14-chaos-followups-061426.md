...
- Make sure closeout writes `data-private/history/mlb-results-2026-06-14.jsonl`.
- Make sure imported side boards actually flow into `mlb_side_backtests`.

Reason:
- today had `14` board picks
- only `0` side-prediction rows imported
- `mlb_side_backtests` still shows `0` rows for this day/model path
