...
| N3 | Tennis replay normalization | SofaScore/Livesport replay legacy tables | `replay_games`, `replay_points` | not_started |  |  | pending | Preserve point/game flow and pressure flags. |
| N4 | Tennis market normalization | Kalshi/Robinhood/FanDuel legacy market tables | `market_snapshots`, `market_contracts`, `market_price_ticks` | not_started |  |  | pending | Contract and tick tables may require additive schema migration. |
...
| N6 | Tennis DuckDB readiness | `sql-tennis.db` typed tables | `duck-tennis.duckdb` | started | 2026-06-02 |  | `build_tennis_duckdb_after_stats_normalization_2026-06-02.json`; zero count mismatches after stats normalization | Rebuild after each validated family. |
...
| Tennis replay | `tennis_sofascore_replay_games`, `tennis_sofascore_replay_points`, `tennis_livesport_replay_games`, `tennis_livesport_replay_points` | `replay_games`, `replay_points` | `pipeline/sources/tennis/normalization/replay.py` | `data-migration/scripts/normalize_tennis_replay.py` | not_started |  |  | pending | pending | Must validate point counts. |
| Tennis markets | `tennis_kalshi_market_candles`, `tennis_kalshi_match_markets`, `tennis_kalshi_open_orderbook_snapshots`, `tennis_prediction_market_snapshots` | `market_snapshots`, `market_contracts`, `market_price_ticks` | `pipeline/sources/tennis/normalization/markets.py` | `data-migration/scripts/normalize_tennis_markets.py` | not_started |  |  | pending | pending | Time-series rows stay in SQLite tables, then DuckDB copies them. |
