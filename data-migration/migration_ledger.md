...
| Empty MLB DB schema | schema plan | `mlb.db` schema tables, migration audit, alias tables, export manifests, required indexes | `data-migration/scripts/create_sport_dbs.mjs --sport mlb` | not_started |  |  | `data-migration/scripts/validate_sport_db.mjs --schema-only mlb` | Phase 1. Must include idempotent dry-run support before writing DB. |
| Empty tennis DB schema | schema plan | `tennis.db` schema tables, migration audit, alias tables, export manifests, required indexes | `data-migration/scripts/create_sport_dbs.mjs --sport tennis` | not_started |  |  | `data-migration/scripts/validate_sport_db.mjs --schema-only tennis` | Phase 1. Must include idempotent dry-run support before writing DB. |
