...
| M9A source policy split | `source_fetch_policies` coarse MLB rows | `sql-mlb.db` source policies/status contract | pending | update `apply_fetch_contract_schema.mjs` for MLB subfamilies | not_started |  |  | pending | Coarse policies currently exist but no MLB status rows exist. |
