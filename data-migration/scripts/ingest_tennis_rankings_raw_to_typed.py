...
from pipeline.sources.tennis.normalization.rankings import (
    insert_rankings,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT / "data-migration" / "reports" / "ingest_tennis_rankings_raw_to_typed_2026-06-02.json",
    )
...
def candidate_files(date: str) -> list[Path]:
    history_path = ROOT / "data-private" / "reference" / "tennis" / "player-rankings-history" / f"{date}.json"
    current_path = ROOT / "data-private" / "reference" / "tennis" / "player-rankings.json"
    if history_path.exists():
        return [history_path]
    if current_path.exists():
        payload = read_json(current_path)
        if payload.get("asOf") == date:
            return [current_path]
    return []
...
def ensure_source_snapshot(con: sqlite3.Connection, file_path: Path, payload: dict[str, Any], date: str) -> str:
    ...
    con.execute(
        ...
        (
            snapshot_id,
            SOURCE_NAME,
            (payload.get("sourceUrls") or [None])[0] if isinstance(payload.get("sourceUrls"), list) else None,
            local_path,
            utc_now(),
            payload.get("asOf") or date,
            sha256_file(file_path),
            ranking_source_notes(payload, local_path, date),
        ),
    )
    return snapshot_id
...
def update_fetch_status(
    ...
    *,
    ...
    now = utc_now()
    status = "success" if source_files > 0 and parsed_rankings > 0 else "missing"
    run_id = f"source-fetch-{SOURCE_NAME}-{date}-{now.replace(':', '-').replace('.', '-')}"
    details = {
        "adapter": "tennis_rankings_raw_to_typed",
        "dry_run": dry_run,
        "source_files": source_files,
        "parsed_rankings": parsed_rankings,
        "unresolved_added": unresolved_added,
        "report_path": sql_path(report_path),
        "note": "Typed parse run for tennis ranking raw archive. This does not perform network fetch.",
    }
    con.execute(
        """
    ...
    con.execute(
        ...
        insert into source_fetch_status (
          ...
          ?, 'unknown', datetime(?, '+24 hours'), null, ?, null, ?, ?, ?)
...
def insert_health_check(con: sqlite3.Connection, *, date: str, status: str, report: dict[str, Any]) -> None:
    ...
    con.execute(
        ...
        """,
        (
            f"tennis-rankings-raw-to-typed:{date}",
            f"tennis_rankings_raw_to_typed:{date}",
            status,
            report["source_rows"],
            report["inserted_rankings"],
            compact_json(report),
            now,
        ),
    )
...
def ingest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with sqlite3.connect(args.source_db) as con:
        ...
        before_rankings = con.execute("select count(*) from rankings").fetchone()[0]
        rows = []
        source_snapshots = []
        counts = {
            "source_rows": 0,
        ...
        for file_path in files:
            ...
            parsed, row_counts = parse_rankings_payload(
                payload,
                resolver,
        ...
        inserted_rankings = 0
        if not args.dry_run:
            inserted_rankings = insert_rankings(con, rows)
        after_unresolved = con.execute("select count(*) from unresolved_entities").fetchone()[0]
        after_rankings = con.execute("select count(*) from rankings").fetchone()[0]
        unresolved_added = after_unresolved - before_unresolved
        report = {
            "generated_at": utc_now(),
            "script": "data-migration/scripts/ingest_tennis_rankings_raw_to_typed.py",
            ...
            **counts,
            "rankings_by_tour": rankings_coverage(rows),
            "inserted_rankings": inserted_rankings,
            "unresolved_rows_added": unresolved_added,
            "before_counts": {
                "rankings": before_rankings,
                "unresolved_entities": before_unresolved,
            },
            "after_counts": {
                "rankings": after_rankings,
                "unresolved_entities": after_unresolved,
            },
            "row_count_delta": {
                "rankings": after_rankings - before_rankings,
                "unresolved_entities": unresolved_added,
            },
            "sample_source_snapshots": source_snapshots[:8],
            "ok": len(files) > 0 and len(rows) > 0,
        }
        ...
        if args.dry_run:
            ...
            append_normalization_event(
                ROOT,
                {
                    "event_id": f"phase9d-tennis-rankings-raw-to-typed-{args.date}-{utc_now().replace(':', '-').replace('.', '-')}",
                    "timestamp": utc_now(),
                    "phase": "9D",
                    "area": "tennis_rankings_raw_to_typed",
                    "source": "data-private/reference/tennis/player-rankings-history",
                    "target": "sql-tennis.db:rankings,source_fetch_status",
                    "parser_module": "pipeline/sources/tennis/normalization/rankings.py",
                    "migration_script": "data-migration/scripts/ingest_tennis_rankings_raw_to_typed.py",
                    "validation": f"{inserted_rankings} ranking rows parsed from {len(files)} source file(s)",
                    "status_from": "mapped",
                    "status_to": "inserted",
                    "report_path": sql_path(args.report),
                    "checksum": None,
                    "notes": compact_json({"rankings_by_tour": report["rankings_by_tour"], "unresolved_added": unresolved_added}),
                },
            )
