...
from pipeline.sources.mlb.normalization.results import normalize_results
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "data-migration" / "reports" / "normalize_mlb_results_2026-06-02.json")
...
def main() -> int:
    ...
    with sqlite3.connect(args.source_db) as con:
        ...
        report = normalize_results(con, date=args.date, dry_run=args.dry_run)
    report.update(
        {
            "generated_at": utc_now(),
            "script": "data-migration/scripts/normalize_mlb_results.py",
    ...
    if not args.dry_run:
        append_normalization_event(
            ROOT,
            {
                "event_id": f"normalize-mlb-results-{utc_now().replace(':', '-').replace('.', '-')}",
                "timestamp": utc_now(),
                "phase": "N9",
                "area": "mlb_results_normalization",
                "source": "legacy_table_rows:mlb_*_outcomes,mlb_*_stats,mlb_*_logs",
                ...
                "parser_module": "pipeline/sources/mlb/normalization/results.py",
                "migration_script": "data-migration/scripts/normalize_mlb_results.py",
                "validation": "pending",
                "status_from": "started",
                "status_to": "inserted",
