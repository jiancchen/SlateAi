...
from pipeline.sources.mlb.normalization.predictions import normalize_predictions
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "data-migration" / "reports" / "normalize_mlb_predictions_2026-06-02.json")
...
def main() -> int:
    ...
    with sqlite3.connect(args.source_db) as con:
        ...
        report = normalize_predictions(con, date=args.date, dry_run=args.dry_run)
    report.update(
        {
            "generated_at": utc_now(),
            "script": "data-migration/scripts/normalize_mlb_predictions.py",
    ...
    if not args.dry_run:
        append_normalization_event(
            ROOT,
            {
                "event_id": f"normalize-mlb-predictions-{utc_now().replace(':', '-').replace('.', '-')}",
                "timestamp": utc_now(),
                "phase": "N17",
