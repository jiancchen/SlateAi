...
from pipeline.sources.mlb.normalization.team_features import SPECS, TEAM_SOURCE_TABLES


SOURCE_TO_TARGET = {source: spec["target"] for source, spec in SPECS.items()}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "data-migration" / "reports" / "validate_mlb_team_features_normalization_2026-06-02.json")
...
def main() -> int:
    args = parse_args()
    placeholders = ",".join("?" for _ in TEAM_SOURCE_TABLES)
    params: list[str] = ["mlb", *TEAM_SOURCE_TABLES]
    ...
    with sqlite3.connect(args.source_db) as con:
        ...
        unresolved = con.execute(
            "select count(*) from unresolved_entities where entity_type = 'mlb_team_feature'"
        ).fetchone()[0]
        errors: list[str] = []
        if source_rows <= 0:
            errors.append("No MLB team feature source rows found.")
        ...
        for table, count in orphan_checks.items():
            if count:
                errors.append(f"{table} has {count} orphan team references.")
        report = {
            "generated_at": utc_now(),
            "script": "data-migration/scripts/validate_mlb_team_features_normalization.py",
            "parser_module": "pipeline/sources/mlb/normalization/team_features.py",
            ...
            "unresolved_team_entities": unresolved,
            "errors": errors,
            "ok": not errors,
        }
        if not errors:
            con.execute(
                ...
                ) values (?, null, 'mlb_team_features_normalization', 'ok', ?, ?, ?, ?)
                """,
                (
                    f"mlb-team-features-normalization-{utc_now().replace(':', '-').replace('.', '-')}",
                    source_rows,
    ...
    append_normalization_event(
        ROOT,
        {
            "event_id": f"validate-mlb-team-features-{utc_now().replace(':', '-').replace('.', '-')}",
            "timestamp": utc_now(),
            "phase": "N14",
            "area": "mlb_team_features_normalization_validation",
            "source": "sql-mlb.db:legacy_table_rows",
            "target": "sql-mlb.db:team_* feature tables",
            "parser_module": "pipeline/sources/mlb/normalization/team_features.py",
            "migration_script": "data-migration/scripts/validate_mlb_team_features_normalization.py",
