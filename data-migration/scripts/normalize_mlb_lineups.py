...
from pipeline.sources.mlb.normalization.lineups import normalize_lineups
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "data-migration" / "reports" / "normalize_mlb_lineups_2026-06-02.json")
...
def main() -> int:
    ...
    with sqlite3.connect(args.source_db) as con:
        ...
        report = normalize_lineups(con, date=args.date, dry_run=args.dry_run)
    report.update(
        {
            "generated_at": utc_now(),
            "script": "data-migration/scripts/normalize_mlb_lineups.py",
    ...
    if not args.dry_run:
        append_normalization_event(
            ROOT,
            {
                "event_id": f"normalize-mlb-lineups-{utc_now().replace(':', '-').replace('.', '-')}",
                "timestamp": utc_now(),
                "phase": "N10",
                "area": "mlb_lineups_normalization",
                "source": "legacy_table_rows:mlb_lineup_*",
                "target": "sql-mlb.db:lineups,lineup_slots,lineup_matchup_snapshots,lineup_shape_snapshots",
                "parser_module": "pipeline/sources/mlb/normalization/lineups.py",
                "migration_script": "data-migration/scripts/normalize_mlb_lineups.py",
                "validation": "pending",
                "status_from": "started",
                "status_to": "inserted",
