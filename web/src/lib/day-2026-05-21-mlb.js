...
} from './mlb-context-2026-05-21.js'
import { teamStoryContextByTeam } from './mlb-story-context-2026-05-21.js'
import { homeRunTargetsByGame } from './day-2026-05-21-home-run-data.js'
import { bullpenChainByTeam, rawGames } from './day-2026-05-21-data.js'
import { lineupBoardsByGameId, lineupMatchupContextByGameId } from './day-2026-05-21-lineups.js'

export const mlbNotes = [
  'The May 21 MLB board is the initial full-day slate built before first pitch, so the early matinee games stay visible instead of getting trimmed out of the desk.',
  ...
  'Four team lineups were officially posted on this pass, with the rest still leaning on projected orders, so the side card is more trustworthy than the HR layer until more batting orders lock.'
]

export const mlbSources = [
  { label: 'MLB probable pitchers', url: 'https://www.mlb.com/probable-pitchers/2026-05-21' },
  {
    label: 'MLB schedule API for May 21, 2026',
    url: 'https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=2026-05-21&hydrate=probablePitcher,team'
  },
  {
  ...
  {
    label: 'ESPN MLB daily batting leaders for May 20, 2026',
    url: 'https://www.espn.com/mlb/stats/dailyleaders/_/date/20260520/type/batting'
  },
  { label: 'MLB starting lineups', url: 'https://www.mlb.com/starting-lineups' },
  { label: 'ScoresAndOdds MLB board', url: 'https://www.scoresandodds.com/mlb?date=2026-05-21' }
]
...
const getWindowLabel = (startMinutes) => {
  ...
  if (startMinutes < 960) return 'Thursday afternoon board'
  if (startMinutes < 1110) return 'Thursday prime board'
  return 'Thursday night board'
}
