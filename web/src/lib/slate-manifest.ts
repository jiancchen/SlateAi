...
export const slateDayManifest: SlateManifestEntry[] = [
  ...
  { id: '2026-05-25', label: 'May 25, 2026', status: 'ready', slateMeta: { date: 'May 25, 2026', isoDate: '2026-05-25' }, summary: { totalGames: 62 } },
  ...
  { id: '2026-05-31', label: 'May 31, 2026', status: 'ready', slateMeta: { date: 'May 31, 2026', isoDate: '2026-05-31' }, summary: { totalGames: 23 } },
  { id: '2026-06-01', label: 'June 1, 2026', status: 'ready', slateMeta: { date: 'June 1, 2026', isoDate: '2026-06-01' }, summary: { totalGames: 8 } },
  { id: '2026-06-04', label: 'June 4, 2026', status: 'ready', slateMeta: { date: 'June 4, 2026', isoDate: '2026-06-04' }, summary: { totalGames: 23 } }
]

const dayModules = import.meta.glob('./day-*.js')
const legacySlateModules = import.meta.glob('./slate.js')

const loaderById: Record<string, () => Promise<any>> = {
  '2026-05-09': legacySlateModules['./slate.js'] as () => Promise<any>,
  ...Object.fromEntries(
    Object.entries(dayModules).map(([path, loader]) => {
      const match = path.match(/day-(\d{4}-\d{2}-\d{2})\.js$/)
...
export const loadSlateDay = async (id: string): Promise<LoadedSlateDay> => {
  ...
  const manifestEntry = slateDayManifest.find((entry) => entry.id === id)

  if (!loader || !manifestEntry) {
    throw new Error(`No slate loader registered for ${id}`)
  }
  ...
  return {
    ...manifestEntry,
    slateMeta: module.slateMeta ?? manifestEntry.slateMeta,
    filters: module.filters ?? ['All'],
    oddsMeta: module.oddsMeta ?? {},
    games,
    sources: module.sources ?? [],
    summary: {
      totalGames: games.length || manifestEntry.summary.totalGames
    }
  }
}
...
const latestActiveOrPastSlate = [...slateDayManifest]
  .filter((day) => day.id <= currentLocalIsoDate())
  .at(-1)

export const defaultSlateDayId = latestActiveOrPastSlate?.id ?? slateDayManifest.at(-1)?.id ?? ''
