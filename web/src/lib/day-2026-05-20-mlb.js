...
import { teamStoryContextByTeam } from './mlb-story-context-2026-05-20.js'
import { homeRunTargetsByGame } from './day-2026-05-20-home-run-data.js'
import { bullpenChainByTeam, rawGames } from './day-2026-05-20-data.js'
import { lineupBoardsByGameId, lineupMatchupContextByGameId } from './day-2026-05-20-lineups.js'

export const mlbNotes = [
  'The May 20 MLB board is a live remaining-games desk built at 10:13 AM Pacific, so the two games that had already started were intentionally removed from the actionable slate.',
  'Today’s emphasis is still cleaner separation between real baseball control and paper-favorite traps: if the starter window, lineup pressure, bridge chain, and team story do not agree, the card should read as volatile instead of pretending certainty.',
  'Nine team lineups were officially posted on this pass, with the rest still leaning on projected orders, so the side card is more trustworthy than the HR layer until more batting orders lock.'
]

export const mlbSources = [
  { label: 'MLB probable pitchers', url: 'https://www.mlb.com/probable-pitchers/2026-05-20' },
  {
    label: 'MLB schedule API for May 20, 2026',
    url: 'https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=2026-05-20&hydrate=probablePitcher,team'
  },
  {
  ...
  {
    label: 'ESPN MLB daily batting leaders for May 19, 2026',
    url: 'https://www.espn.com/mlb/stats/dailyleaders/_/date/20260519/type/batting'
  },
  { label: 'MLB starting lineups', url: 'https://www.mlb.com/starting-lineups' },
  { label: 'ScoresAndOdds MLB board', url: 'https://www.scoresandodds.com/mlb?date=2026-05-20' }
]
...
const getWindowLabel = (startMinutes) => {
  if (startMinutes < 720) return 'Wednesday matinee'
  if (startMinutes < 960) return 'Wednesday afternoon board'
  if (startMinutes < 1110) return 'Wednesday prime board'
  return 'Wednesday night board'
}
...
const remainingGameCutoffMinutes = 613

export const mlbGames = rawGames.filter((game) => game.startMinutes > remainingGameCutoffMinutes).map((game) =>
